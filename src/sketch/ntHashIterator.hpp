...
public:
  ...
  /**
    ...
    */
  ntHashIterator(const std::string &seq, unsigned h, unsigned k, bool rc) : m_seq(seq), m_h(h), m_k(k), m_rc(rc), m_hVec(new uint64_t[h]), m_pos(0)
  {
    init();
  }
  ...
  void init()
  {
    ...
    unsigned locN = 0;
    while (m_pos < m_seq.length() - m_k + 1 && (m_rc ? !NTMC64(m_seq.data() + m_pos, m_k, m_h, m_fhVal, m_rhVal, locN, m_hVec)
