...
Reference::Reference()
    : _bbits(def_bbits), _sketchsize64(def_sketchsize64), _use_rc(true),
      _reads(false), _seq_size(0), _densified(false) {}

Reference::Reference(const std::string &name, SeqBuf &sequence,
                     const KmerSeeds &kmers, const size_t sketchsize64,
                     const bool codon_phased, const bool use_rc,
                     const uint8_t min_count, const bool exact)
    : _name(name), _bbits(def_bbits), _sketchsize64(sketchsize64),
      _use_rc(use_rc), _reads(sequence.is_reads()), _seq_size(0),
      _densified(false) {
  if (sequence.nseqs() == 0) {
...
Reference::Reference(const std::string &name, const size_t bbits,
                     const size_t sketchsize64, const size_t seq_size,
                     const std::vector<double> bases,
                     const unsigned long int missing_bases, const bool reads)
...
double Reference::jaccard_dist(Reference &query, const int kmer_len,
                               const double random_jaccard) {
  size_t intersize =
      calc_intersize(&this->get_sketch(kmer_len), &query.get_sketch(kmer_len),
                     _sketchsize64, _bbits);
...
Reference::core_acc_dist<RandomMC>(Reference &query, const RandomMC &random);
template std::tuple<float, float> Reference::core_acc_dist<std::vector<double>>(
    Reference &query, const std::vector<double> &random);
...
const std::vector<uint64_t> &Reference::get_sketch(const int kmer_len) const {
  try {
    ...
    throw std::runtime_error("Kmer length " + std::to_string(kmer_len) +
                             " not found in sketch " + _name);
  }
}
