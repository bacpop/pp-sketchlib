  ...
  try {
    ...
    for (int i = 0; i < dists.size(); ++i) {
      if (dists(i) < tolerance) {
        ...
        slopes = (kmer_truncation.transpose() * kmer_truncation).ldlt().solve(kmer_truncation.transpose() * (dists_truncation.array().log()));
        break;
      } else if (i == dists.size() - 1) {
        // See https://eigen.tuxfamily.org/dox/group__LeastSquares.html
        slopes = (kmers.transpose() * kmers).ldlt().solve(kmers.transpose() * (dists.array().log()));
      }
    }
...
std::tuple<float, float> regress_kmers(Reference *r1, Reference *r2,
  ...
  for (unsigned int i = 0; i < dists.size(); ++i) {
    dists[i] = r1->jaccard_dist(*r2, kmers(i, 1), random[i]);
  }
  return (fit_slope(kmers, dists, r1, r2));
}
