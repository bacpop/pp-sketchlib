  ...
  try {
    ...
    for (int i = 0; i < dists.size(); ++i) {
      if (dists(i) < tolerance) {
        ...
        slopes = (kmer_truncation.transpose() * kmer_truncation).ldlt().solve(kmer_truncation.transpose() * log(dists_truncation));
        break;
      } else if (i == dists.size() - 1) {
        // See https://eigen.tuxfamily.org/dox/group__LeastSquares.html
        slopes = (kmers.transpose() * kmers).ldlt().solve(kmers.transpose() * log(dists));
      }
    }
