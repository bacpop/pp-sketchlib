...
struct dist_params {
  ...
  long n_samples;
  unsigned int chunks;
  size_t calc_per_chunk;
  unsigned int num_big_chunks;
...
dist_params cuda_dists_init(const std::vector<Reference> &ref_sketches,
  ...
  double est_size =
      (bbits * sketchsize64 * kmer_lengths.size() * params.n_samples *
  ...
  if (est_size > mem_free * (1 - mem_epsilon) {
    if (params.self) {
      ...
      if (est_size > mem_free * (1 - mem_epsilon)) {
        params.chunks = floor((est_size * 2) / (mem_free * (1 - mem_epsilon))) + 1;
      }
      params.calc_per_chunk = params.n_samples / params.chunks;
      params.num_big_chunks = params.n_samples % params.chunks;
      params.total_chunks = (params.chunks * (params.chunks + 1)) >> 1;
    } else {
...
NumpyMatrix query_db_cuda(std::vector<Reference> &ref_sketches,
  ...
  if (params.self) {
    ...
    for (unsigned int chunk_i = 0; chunk_i < params.chunks; chunk_i++) {
      sketch_subsample.query_size = params.calc_per_chunk;
      if (chunk_i < params.num_big_chunks) {
        sketch_subsample.query_size++;
      }

      sketch_subsample.ref_offset = sketch_subsample.query_offset;
      for (unsigned int chunk_j = chunk_i; chunk_j < params.chunks; chunk_j++) {
        if (total_chunks > 1) {
          std::cerr << "Running chunk " << ++chunk_count << " of "
                    << total_chunks << std::endl;
        }

        sketch_subsample.ref_size = params.calc_per_chunk;
        if (chunk_j < params.num_big_chunks) {
  ...
  if (params.self && params.chunks > 1) {
