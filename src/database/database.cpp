...
void Database::add_sketch(const Reference &ref) {
  ...
  HighFive::Attribute sketch_size_a = sketch_group.createAttribute<size_t>(
      "sketchsize64", HighFive::DataSpace::From(ref.sketchsize64()));
  sketch_size_a.write(ref.sketchsize64());
  HighFive::Attribute bbits_a = sketch_group.createAttribute<size_t>(
      "bbits", HighFive::DataSpace::From(ref.bbits()));
  bbits_a.write(ref.bbits());
  HighFive::Attribute length_a = sketch_group.createAttribute<size_t>(
      "length", HighFive::DataSpace::From(ref.seq_length()));
  length_a.write(ref.seq_length());
  HighFive::Attribute missing_a =
  ...
  HighFive::Attribute bases_a = sketch_group.createAttribute<double>(
      "base_freq", HighFive::DataSpace::From(bases));
  bases_a.write(bases);
  const std::vector<size_t> kmer_lengths = ref.kmer_lengths();
  HighFive::Attribute kmers_a = sketch_group.createAttribute<int>(
      "kmers", HighFive::DataSpace::From(kmer_lengths));
  ...
  for (auto kmer_it = kmer_lengths.cbegin(); kmer_it != kmer_lengths.cend();
    ...
    // HighFive::DataSet sketch_dataset =
    // _h5_file.createDataSet<uint64_t>(dataset_name,
    // HighFive::DataSpace::From(sketch), save_properties);
    HighFive::DataSet sketch_dataset = _h5_file.createDataSet<uint64_t>(
        dataset_name, HighFive::DataSpace::From(sketch));
    sketch_dataset.write(sketch);

    HighFive::Attribute kmer_size_a = sketch_dataset.createAttribute<int>(
        "kmer-size", HighFive::DataSpace::From(*kmer_it));
...
void Database::save_random(const RandomMC &random) {
  ...
  if (!_h5_file.exist("random")) {
    ...
    save_hash<std::string, uint16_t>(random.cluster_table(), random_group,
                                     "table");
    ...
    HighFive::Attribute k_min_a = random_group.createAttribute<unsigned int>(
        "k_min", HighFive::DataSpace::From(k_min));
    k_min_a.write(k_min);
...
HighFive::File open_h5(const std::string &filename, const bool write) {
  return (HighFive::File(filename.c_str(), write ? HighFive::File::ReadWrite
...
Database new_db(const std::string &filename, const bool use_rc,
  ...
  // Restrict scope of HDF5 so it is closed before reopening it
  {
    HighFive::File h5_file =
        HighFive::File(filename.c_str(), HighFive::File::Overwrite);
