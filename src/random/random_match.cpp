...
RandomMC::RandomMC()
    : _n_clusters(0), _no_adjustment(true), _no_MC(false), _use_rc(false),
      _min_k(0), _max_k(default_max_k) {}

RandomMC::RandomMC(const bool use_rc)
    : _n_clusters(0), _no_adjustment(false), _no_MC(true), _use_rc(use_rc),
      _min_k(0), _max_k(default_max_k) {}

// Constructor - generates random match chances by Monte Carlo, sampling
// probabilities from the input sketches
RandomMC::RandomMC(const std::vector<Reference> &sketches,
                   unsigned int n_clusters, const unsigned int n_MC,
                   const bool codon_phased, const bool use_rc,
  ...
  while (kmer_size <= max_kmer) {
    double match_chance = default_adjustment.random_match(
        sketches[0], 0, sketches[0].seq_length(), kmer_size);
  ...
  Xoshiro generator(1);
#pragma omp parallel for collapse(2) firstprivate(generator) schedule(static)  \
...
#endif
      // Make the sequence
      SeqBuf random_seq(generate_random_sequence(_cluster_centroids.row(r_idx),
                                                 sketches[0].seq_length(),
                                                 use_rc, generator));
      // Sketch it
      random_seqs[r_idx].push_back(Reference(
          "random" + std::to_string(r_idx * n_MC + copies), random_seq,
          kmer_seeds, sketchsize64, codon_phased, use_rc, 0, false));
    }
  }

  RandomMC no_adjust;
  // printf("kmer\tMC\tformula\n");
  for (auto kmer_it = kmer_lengths.begin(); kmer_it != kmer_lengths.end();
       kmer_it++) {
    ...
    for (unsigned int i = 0; i < n_clusters; i++) {
      for (unsigned int j = i; j < n_clusters; j++) {
        double dist_sum = 0;
#pragma omp parallel for simd collapse(2) schedule(static) reduction(+ \
                                                                     : dist_sum)
        for (unsigned int copy_i = 0; copy_i < n_MC; copy_i++) {
          for (unsigned int copy_j = 0; copy_j < n_MC; copy_j++) {
            dist_sum += random_seqs[i][copy_i].jaccard_dist(
                random_seqs[j][copy_j], *kmer_it, no_adjust);
          }
        }
        int dist_count = n_MC * n_MC;
        // Diagonal contains n_MC self comparisons (== 1) which need to be
        // removed
        if (i == j) {
          dist_sum -= n_MC;
          dist_count -= n_MC;
        ...
        // printf("%lu\t%f\t%f\n", *kmer_it, matches(i, j),
        // default_adjustment.random_match(random_seqs[i][0],
        // random_seqs[j][0],
        //												*kmer_it));
      }
    }
...
std::vector<double>
RandomMC::random_matches(const Reference &r1, const uint16_t q_cluster_id,
                         const size_t q_length,
                         const std::vector<size_t> &kmer_lengths) const {

  std::vector<double> random;
  for (auto kmer_len = kmer_lengths.cbegin(); kmer_len != kmer_lengths.cend();
       ++kmer_len) {
...
std::vector<uint16_t>
RandomMC::lookup_array(const std::vector<Reference> &sketches) const {
...
// access: kmer_idx * kmer_stride + ref_idx * inner_stride + query_idx *
// outer_stride
FlatRandom RandomMC::flattened_random(const std::vector<size_t> &kmer_lengths,
      ...
      if (_no_MC) {
        ...
        if (k < _min_k) {
          throw std::runtime_error(
              "Trying to choose a k-mer length below the minimum allowed\n");
        } else if (k <= _max_k) {
          std::copy(_matches.at(k).data(), _matches.at(k).data() + matrix_size,
...
double csrs(const size_t k, const bool use_rc, const size_t l1,
  ...
  // int rc_factor = _use_rc ? 2 : 1; // If using the rc, may randomly match on
  // the other strand size_t match_chance = rc_factor * std::pow(N_BASES,
  // (double)-kmer_len)
  // float j1 = r1.seq_length() / (r1.seq_length() + rc_factor *
  // std::pow(N_BASES, (double)-kmer_len)); float j2 = r2.seq_length() /
  // (r2.seq_length() + rc_factor * std::pow(N_BASES, (double)-kmer_len));
...
std::vector<std::string> generate_random_sequence(const Eigen::VectorXf &base_f,
                                                  ...
                                                  const bool use_rc,
                                                  Xoshiro &generator) {
  std::discrete_distribution<int> base_dist{base_f[0], base_f[1], base_f[2],
                                            base_f[3]};
